...
gulp.task('image', function () {
    return gulp.src('ecommerce_dev/images/*')
        .pipe(imagemin())
...
gulp.task('js', function () {
    return gulp.src(['ecommerce_dev/js/**/*.*','ecommerce_dev/js/*.js']) // al folder al feh al files bt3te
...
gulp.task('css', function () {
    return gulp.src('ecommerce_dev/scss/style.scss') 
        .pipe(sourcemaps.init()) //lazem yt7t b3d al source 3la tol 
        .pipe(sass({ outputStyle: 'expand' })) //hytl3 al output style md3'ot
...
gulp.task('html', function () {
  return gulp.src('ecommerce_dev/*.html') 
...
gulp.task('watch', function () {
 ...
 //  livereload.listen(); // by3ml listen lle 3mlhom livereload
    gulp.watch('ecommerce_dev/scss/**/*.scss', gulp.series('css')) //ay folder gwa al css w ay file amtddao css 
    gulp.watch(['ecommerce_dev/js/**/*.*','ecommerce_dev/js/*.js'], gulp.series('js')) //ay folder gwa al css w ay file amtddao css 
    gulp.watch('ecommerce_dev/*.html', gulp.series('html'))
    gulp.watch('ecommerce_dev/images/*', gulp.series('image'))
